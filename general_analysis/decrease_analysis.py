...
def analyze_quantity_decreased_for_all_tables_with_price(db_config):
    ...
    try:
        ...
        cursor.execute("""
        CREATE TABLE IF NOT EXISTS analyzed_quantity_decreased_pairs (
            table1 VARCHAR(255),
            table2 VARCHAR(255),
        ...
        cursor.execute("SELECT table1, table2 FROM analyzed_quantity_decreased_pairs WHERE analyzed = TRUE")
        ...
        for i in range(len(product_tables) - 1):
            ...
            SELECT
                ...
                (p1.quantity_in_stock - p2.quantity_in_stock) AS quantity_decreased
            FROM
                {table1} p1
            JOIN
                {table2} p2 ON p1.id = p2.id
            WHERE
                p1.quantity_in_stock > p2.quantity_in_stock;
            """
            ...
            if analysis_results:
                ...
                new_table_name = f"decreased_{table1}_{table2}"
                ...
                cursor.execute(f"INSERT INTO analyzed_quantity_decreased_pairs (table1, table2, analyzed) VALUES ('{table1}', '{table2}', TRUE)")
...
if __name__ == "__main__":
    analyze_quantity_decreased_for_all_tables_with_price(db_config)
